...
//string
    string private _name = "MetaSkylines";
    string private _symbol = "MSKY";
//bool
...
//address
    ...
    address public _MarketingWalletAddress = 0xcB66e18a38Bf9eFB00b13b4CA9f9dd1A9bAc09cb;
    address public _PoolRewardWalletAddress = 0xB2E5A8416FeF6192ffB138C6Ab8dd9Faa00D70C0;
    address public _DevelopmentWalletAddress = 0x923cF51E9c35Ff79fBdA8Db93f70a9d36340d349;
    address[] private _excluded;
//uint
    uint public buyReflectionFee = 0;
    uint public sellReflectionFee = 0;
    uint public buyLiqFee = 7;
    uint public sellLiqFee = 7;

    uint public buyMarketingFee = 2;
    uint public sellMarketingFee = 2;
    uint public buyPoolRewardFee = 4;
    uint public sellPoolRewardFee = 4;
    uint public buyDevelopmentFee = 2;
    uint public sellDevelopmentFee = 2;
    uint public buyBusinessFee = buyMarketingFee + buyPoolRewardFee + buyDevelopmentFee;
    uint public sellBusinessFee = sellMarketingFee + sellPoolRewardFee + sellDevelopmentFee;
	
    uint public buyFee = buyReflectionFee + buyLiqFee + buyBusinessFee;
    uint public sellFee = sellReflectionFee + sellLiqFee + sellBusinessFee;
    uint public buySecondsLimit = 2;
...
// constructor
    constructor() {
        ...
        excludedFromFees[_PoolRewardWalletAddress] = true;
        excludedFromFees[_DevelopmentWalletAddress] = true;
        //spawn pair
    ...
    function totalReflectionReward() public view returns (uint256) {
        return _tReflectionRewardTotal;
    }
    ...
    function setFees() private {
		buyBusinessFee = buyMarketingFee + buyPoolRewardFee + buyDevelopmentFee;
		sellBusinessFee = sellMarketingFee + sellPoolRewardFee + sellDevelopmentFee;
        buyFee = buyReflectionFee + buyLiqFee + buyBusinessFee;
        sellFee = sellReflectionFee + sellLiqFee + sellBusinessFee;
    }
    ...
    function setPoolRewardFee(uint buy, uint sell) external onlyOwner() {
        buyPoolRewardFee = buy;
        sellPoolRewardFee = sell;
        setFees();
    }
    ...
    function _reflectFee(uint256 rFee, uint256 tFee) private {
        ...
        _tReflectionRewardTotal = _tReflectionRewardTotal.add(tFee);
    }

    function _getValues(uint256 tAmount) private view returns (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity, uint256 tBusiness) {
        (tTransferAmount, tFee, tLiquidity, tBusiness) = _getTValues(tAmount);
        (rAmount, rTransferAmount, rFee) = _getRValues(tAmount, tFee, tLiquidity, tBusiness, _getRate());
        return (rAmount, rTransferAmount, rFee, tTransferAmount, tFee, tLiquidity, tBusiness);
    }

    function _getTValues(uint256 tAmount) private view returns (uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity, uint256 tBusiness) {
        tFee = calculateReflectionFee(tAmount);
        tLiquidity = calculateLiquidityFee(tAmount);
        tBusiness = calculateBusinessFee(tAmount);
        tTransferAmount = tAmount.sub(tFee).sub(tLiquidity).sub(tBusiness);
        return (tTransferAmount, tFee, tLiquidity, tBusiness);
    }

    function _getRValues(uint256 tAmount, uint256 tFee, uint256 tLiquidity, uint256 tBusiness, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
        uint256 rAmount = tAmount.mul(currentRate);
        uint256 rFee = tFee.mul(currentRate);
        uint256 rLiquidity = tLiquidity.mul(currentRate);
        uint256 rBusiness = tBusiness.mul(currentRate);
        uint256 rTransferAmount = rAmount.sub(rFee).sub(rLiquidity).sub(rBusiness);
    ...
    function _takeBusiness(uint256 tBusiness) private {
        uint256 currentRate =  _getRate();
        uint256 rBusiness = tBusiness.mul(currentRate);
        _rOwned[address(this)] = _rOwned[address(this)].add(rBusiness);
        if(_isExcluded[address(this)])
            _tOwned[address(this)] = _tOwned[address(this)].add(tBusiness);
    }
    ...
    function calculateBusinessFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_BusinessFee).div(
            10**2
        );
    }
    function setOldFees() private {
        _OldReflectionFee = _ReflectionFee;
        _OldBusinessFee = _BusinessFee;
        _OldLiqFee = _LiqFee;
    }
    ...
    function sendToWallet(uint amount) private {
        ...
        uint256 poolreward_part = amount * sellPoolRewardFee / 100;
        ...
        (bool success2, ) = payable(_PoolRewardWalletAddress).call{value: poolreward_part, gas: 30000}("");
        (bool success3, ) = payable(_DevelopmentWalletAddress).call{value: development_part, gas: 30000}("");
    }
...
//set functions
    ...
    function setpoolrewardAddress(address _value) external onlyOwner {
        _PoolRewardWalletAddress = _value;
    }
...
// operational functions
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity, uint256 tBusiness) = _getValues(tAmount);
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
        _rOwned[recipient] = _rOwned[recipient].add(rTransferAmount);
        _takeLiquidity(tLiquidity);
        _takeBusiness(tBusiness);
